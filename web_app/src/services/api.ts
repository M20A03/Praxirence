...
export const api = {
  ...
  async requestDoctorEmailOtp(email: string, name?: string): Promise<{ success: boolean; message: string }> {
    const res = await fetch(`${API_BASE_URL}/auth/doctor/email-otp/request`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async verifyDoctorEmailOtp(email: string, code: string): Promise<AuthResponse> {
    const res = await fetch(`${API_BASE_URL}/auth/doctor/email-otp/verify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async requestPatientEmailOtp(email: string, name?: string): Promise<{ success: boolean; message: string }> {
    const res = await fetch(`${API_BASE_URL}/auth/patient/email-otp/request`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: email.trim(), name: name?.trim() }),
    });
  ...
  async verifyPatientEmailOtp(email: string, code: string): Promise<AuthResponse> {
    const res = await fetch(`${API_BASE_URL}/auth/patient/email-otp/verify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: email.trim(), code: code.trim() }),
    });
  ...
  async requestPatientOtp(phone: string, channel: 'sms' = 'sms'): Promise<{ success: boolean; message: string; demo_code?: string }> {
